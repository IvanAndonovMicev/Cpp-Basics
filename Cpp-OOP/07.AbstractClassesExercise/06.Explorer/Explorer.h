...
public:
	...
	std::vector < std::shared_ptr<FileSystemObject>>::const_iterator begin() const override { return this->rootObjects.begin(); }

	std::vector < std::shared_ptr<FileSystemObject>>::const_iterator end() const override { return this->rootObjects.end(); }
...
class Explorer {
	...
	std::shared_ptr<Shortcuts> shortcut;
...
public:
	...
	void createFile(const std::string& filename, const std::string& contents) {
		...
		file->setParent(std::dynamic_pointer_cast<FileSystemObject>(currentDirectory));
	}
	...
	void createShortcut(const std::string& name) {
		if (!shortcut) {
			shortcut = std::make_shared<Shortcuts>();
			root->add(shortcut);
		}

		shortcut->add(find(name));
	}

	bool navigateSingleState(std::string path) {
		if (path == "..") {
	...
	bool navigate(const std::vector<std::string>& path, std::vector<std::string>::iterator end) {
		for (auto i = path.begin(); i != end; i++) {
			bool navigated = this->navigateSingleState(*i);
			if (!navigated) {
				return false;
			}
		}
